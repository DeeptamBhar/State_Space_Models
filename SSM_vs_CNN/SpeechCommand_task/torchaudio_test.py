...
# ==========================================
# DEPENDENCY BYPASS HAC
# Intercepts torchaudio to prevent torchcodec CUDA crashes
# ==========================================
def safe_audio_load(filepath, **kwargs):
    # Read the audio file directly into a numpy array
    data, samplerate = sf.read(filepath, dtype='float32')
    
    # torchaudio expects the shape to be (Channels, Frames)
    if data.ndim == 1:
        data = data.reshape(-1, 1)
    
    # Convert numpy array to PyTorch tensor and return
    return torch.from_numpy(data.T), samplerate

# Override the official function with our safe version
torchaudio.load = safe_audio_load
# ==========================================


class SpeechCommandsSubset(Dataset):
    def __init__(self, subset="training", target_length=4000):
        print(f"Loading {subset} dataset into RAM. This will take ~60 seconds...")
        ...
        # Pre-allocate lists for RAM caching
        self.audio_tensors = []
        self.target_tensors = []

        # Load everything into memory ONCE
        for i in tqdm(range(len(self.dataset)), desc=f"Caching {subset}"):
            waveform, _, label, _, _ = self.dataset[i]
            ...
            self.target_tensors.append(self.label_to_index[label])

    def __len__(self):
        return len(self.audio_tensors)

    def __getitem__(self, idx):
        # Instantly return from RAM, zero disk access
        return self.audio_tensors[idx], self.target_tensors[idx]

def get_audio_dataloaders(batch_size=64, num_workers=0):
    train_dataset = SpeechCommandsSubset(subset="training")
    test_dataset = SpeechCommandsSubset(subset="testing")
...
class TCNLayer(nn.Module):
    ...
    def forward(self, x):
        # x expected shape: (Batch, Length, d_model)
        residual = x
        x = self.norm(x)
        # Permute for Conv1d: (Batch, d_model, Length)
        x = x.permute(0, 2, 1)
        x = self.conv(x)
        x = x.permute(0, 2, 1)
        x = self.activation(x)
...
class PureMamba(nn.Module):
    """Model 1: Pure Mamba with N layers."""
    def __init__(self, d_model=64, num_layers=4, num_classes=10):
        super().__init__()
...
class PureTCN(nn.Module):
    """Model 2: Pure TCN. M must be >= 8 to cover a 1024 sequence length."""
    def __init__(self, d_model=64, num_layers=8, kernel_size=3, num_classes=10):
        super().__init__()
        self.embedding = nn.Linear(1, d_model)
...
class HybridParallelBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Branch 1: Mamba (Global)
        mamba_out = self.mamba(x_norm)
        
        # Branch 2: TCN (Local/Regional based on dilation)
...
class HybridParallelMambaTCN(nn.Module):
    """Model 3: Stacked Parallel Hybrid blocks."""
    def __init__(self, d_model=64, num_layers=4, kernel_size=3, num_classes=10):
        super().__init__()
        self.embedding = nn.Linear(1, d_model)
...
# Assume the classes PureMamba, PureTCN, and HybridParallelMambaTCN 
# from the previous implementation are defined here.
...
def train_and_evaluate(model, name, train_loader, test_loader, epochs=30, device='cuda'):
    """
    ...
    save_filename = f"torchaudio_{name.replace(' ', '_').replace('(', '').replace(')', '').lower()}_best.pth"
...
if __name__ == "__main__":
    ...
    # Using 8 layers of Pure Mamba at a standard 128 width
    baseline_d_model = 64
    baseline_layers = 8
    
    baseline_model = PureMamba(d_model=baseline_d_model, num_layers=baseline_layers, num_classes=35) 
    ...
    # TCN strictly requires 11 layers to achieve a >1024 receptive field
    tcn_layers = 11
    
    # Calculate unadjusted parameters first to show the discrepancy
    unadjusted_tcn = PureTCN(d_model=baseline_d_model, num_layers=tcn_layers, num_classes=35)
    print(f"\nUnadjusted TCN (11 Layers, d_model={baseline_d_model}): {count_parameters(unadjusted_tcn):,}")
    ...
    # Keeping depth at 8 layers to match the Mamba baseline depth
    hybrid_layers = 8
    
    unadjusted_hybrid = HybridParallelMambaTCN(d_model=baseline_d_model, num_layers=hybrid_layers, num_classes=35)
    print(f"\nUnadjusted Hybrid (8 Layers, d_model={baseline_d_model}): {count_parameters(unadjusted_hybrid):,}")
    ...
    train_loader, test_loader = get_audio_dataloaders(batch_size=64)  # Reduced batch size for longer sequences
    
    # Instantiate the balanced models derived from the previous parameter search
    models = {
        f"Pure Mamba ({baseline_d_model} width)": PureMamba(d_model=baseline_d_model, num_layers=baseline_layers, num_classes=35),
        f"Pure TCN ({matched_tcn_d} width)": PureTCN(d_model=matched_tcn_d, num_layers=tcn_layers, kernel_size=3, num_classes=35),
        f"Hybrid Parallel ({matched_hybrid_d} width)": HybridParallelMambaTCN(d_model=matched_hybrid_d, num_layers=hybrid_layers, kernel_size=3, num_classes=35)
    }
