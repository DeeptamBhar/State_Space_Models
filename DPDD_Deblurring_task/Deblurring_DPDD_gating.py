...
class CNNMambaBlock(nn.Module):
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2):
        ...
        self.fusion_attention = nn.Sequential(
            ...
            nn.Conv2d(d_model * 2, d_model // 2, kernel_size=1),
            nn.GELU(),
            nn.Conv2d(d_model // 2, d_model * 2, kernel_size=1),
            nn.Sigmoid()
        )
        self.fusion_project = nn.Conv2d(d_model * 2, d_model, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Gated Fusion
        concat_features = torch.cat([x_global_c, x_local_c], dim=1) # Shape: (B, 2*d_model, H, W)
        attention_weights = self.fusion_attention(concat_features)
        fused_c = concat_features * attention_weights
        out_c = self.fusion_project(fused_c)
