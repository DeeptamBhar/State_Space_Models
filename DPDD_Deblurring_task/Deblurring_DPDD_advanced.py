...
class BlurDataset(Dataset):
    def __init__(self, root_dir, split='train_c', crop_size=256):
        ...
        self.transform = transforms.Compose([
            ...
            transforms.RandomHorizontalFlip(p=0.5) if 'train' in split else nn.Identity(),
            transforms.RandomVerticalFlip(p=0.5) if 'train' in split else nn.Identity(),
            transforms.ToTensor()
        ])
...
class HybridLoss(nn.Module):
    def __init__(self, alpha=0.84, fft_weight=0.1):
        super().__init__()
        self.alpha = alpha
    ...
    def forward(self, pred, target):
        ...
        spatial_loss = (self.alpha * ssim_loss) + ((1 - self.alpha) * self.l1(pred, target))
        
        # 2. Frequency (FFT) Loss
        # Convert images to frequency domain and calculate L1 difference
        pred_fft = torch.fft.rfft2(pred, norm='ortho')
        target_fft = torch.fft.rfft2(target, norm='ortho')
        fft_loss = self.l1(pred_fft.abs(), target_fft.abs())
        
        return spatial_loss + (self.fft_weight * fft_loss)
...
class CNNMambaBlock(nn.Module):
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2):
        ...
        # --- The Automated Gate ---
        # Initialize at 2.0 so sigmoid(2.0) starts near 0.88 (mostly ON)
        self.cnn_gate = nn.Parameter(torch.tensor(2.0))
    ...
    def forward(self, x):
        ...
        # Global Pathway (Mamba)
        x_flat = rearrange(x, 'b h w c -> b (h w) c')
        x_global = self.mamba(self.norm_global(x_flat))
        x_global_c = rearrange(x_global, 'b (h w) c -> b c h w', h=H, w=W)
        
        # Local Pathway (CNN)
        x_local_c = self.cnn(self.norm_local(x_c))
        
        # Apply the learned automated gate (Sigmoid keeps it between 0 and 1)
        gate_value = torch.sigmoid(self.cnn_gate)
        out_c = x_global_c + (x_local_c * gate_value)
        
        # Residual
        out_c = out_c + (x_c * self.scale)
        return rearrange(out_c, 'b c h w -> b h w c')
...
def train_model(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (blur, sharp) in enumerate(train_loader):
            ...
            with autocast():
                pred = model(blur)
                reconstruction_loss = criterion(pred, sharp)
                
                # --- Calculate the Sparsity Penalty (The Tax) ---
                sparsity_loss = 0.0
                sparsity_weight = 0.1  # Tune this: Higher = drops CNN faster
                
                # Loop through all modules and find our custom gates
                for module in model.modules():
                    if isinstance(module, CNNMambaBlock):
                        sparsity_loss += torch.sigmoid(module.cnn_gate)
                
                # The optimizer must now balance fixing the image vs paying the tax
            ...
            if batch_idx % 100 == 0:
                logger.info(f"Epoch [{epoch+1}/{args.epochs}] Batch [{batch_idx}/{len(train_loader)}] Loss: {total_loss.item():.4f}")
        ...
        # --- NEW: Track the Automated Gates ---
        gate_values = [torch.sigmoid(m.cnn_gate).item() for m in model.modules() if isinstance(m, CNNMambaBlock)]
        avg_gate = sum(gate_values) / len(gate_values) if gate_values else 0.0

        scheduler.step()
        
        # --- UPDATED: Print the Avg CNN Gate in the log ---
        logger.info(f"Epoch {epoch+1} Completed | Train Loss: {avg_train_loss:.4f} | Avg CNN Gate: {avg_gate:.3f} | Val PSNR: {avg_psnr:.2f} | Val LPIPS: {avg_lpips:.4f}")
